...
# Function 2: Calculate maximum deviation from a plane for a given window
...
def collect_velocities(label_data, data, folder_name, characteristic_name,plane_z):
    ...
    for _, row in label_data.iterrows():
        ...
        z_velocity = calculate_max_deviation_from_plane(extracted_data, plane_z)
        ...
        print(f"deviation for row {row_number} (label {label}): {z_velocity}")
...
def plot_per_participant(all_velocities, participant_count=8, task_name="", window_size=12):
    ...
    plt.ylabel('Deviation from plane (cm)', fontsize=36)
    ...
    plt.legend(handles=legend_handles, loc='upper left', fontsize=24)  # Change legend location to lower right
...
def plot_combined_drops(all_intended, all_unintended, task_name=""):
    ...
    plt.grid(axis='y')  # Add horizontal grid lines
    plt.title(f'Deviation in {task_name} for All Participants', fontsize=16)
...
def plot_mean_z_velocity(all_velocities, participant_count=8, task_name=""):
    ...
    # Add labels and title
    plt.xticks(fontsize=36)  # Increase x-tick font size
    plt.ylabel('Deviation from plane (cm)', fontsize=36)
    plt.grid(axis='y')  # Add horizontal grid lines
    #plt.title(f'Mean Deviation for each participants in {task_name}', fontsize=16)
...
for file_id, data_path, label_path in data_files:
    ...
    plane_z = calibrate_z + 0.55   # type: ignore
    ...
    velocities = collect_velocities(label_data, data, output_dir, characteristic_name,plane_z)
