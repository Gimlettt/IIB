...
slider_events = data_df[data_df.iloc[:, 1].str.contains("Slider", na=False)].iloc[:, 1]
slider_events = slider_events[~slider_events.str.contains("Training")]#filter out the training data
task_start_indices = slider_events[slider_events.str.contains("STARTED TASK Sliders")].index
task_end_indices = slider_events[slider_events.str.contains("FINISHED TASK Sliders")].index
...
for start, end in zip(task_start_indices, task_end_indices):
    ...
    for row_number in task_released_indices:
        # Find the nearest score event before the "RELEASED" event
        corresponding_score = score_events.iloc[index_score]
        index_score += 1
        print(f"Score for row {row_number}: {corresponding_score}")
    ...
    "label": label,
    "score_event": corresponding_score
})
