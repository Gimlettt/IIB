...
def plot_combined_tasks(all_velocities, save_path):
    # 1) specify your desired order
    TASK_ORDER = ['drag', 'slider', 'sketching']
    # 2) map to the short labels you use
    ...
    # 3) iterate in that fixed order
    for task in TASK_ORDER:
        vel = all_velocities[task]
        short = SHORT[task]
        labels.append(f"{short}_I")
        labels.append(f"{short}_U")
        data.append(vel['intended'])
        data.append(vel['unintended'])
    ...
    for i, patch in enumerate(box['boxes']):
        patch.set_facecolor('#1f77b4' if i % 2 == 0 else '#ff7f0e')
